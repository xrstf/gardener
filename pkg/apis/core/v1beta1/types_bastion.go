...
type BastionSpec struct {
	...
	ShootRef BastionShootReference `json:"shootRef" protobuf:"bytes,1,opt,name=shootRef"`
	// SSHPublicKey is the user's public key.
	SSHPublicKey string `json:"sshPublicKey" protobuf:"bytes,2,opt,name=sshPublicKey"`
	// Ingress controls from where the creation bastion host should be reachable.
	Ingress []BastionIngressPolicy `json:"ingress" protobuf:"bytes,2,opt,name=ingress"`
}

// BastionShootReference defines the target shoot for a Bastion.
type BastionShootReference struct {
...
type BastionStatus struct {
	...
	SeedName string `json:"seedName,omitempty" protobuf:"bytes,1,opt,name=seedName"`
	// ProviderType is cloud provider used by the referenced Shoot.
	ProviderType string `json:"providerType,omitempty" protobuf:"bytes,2,name=providerType"`
	// Ingress holds the public IP and/or hostname of the bastion instance.
	Ingress *corev1.LoadBalancerIngress `json:"ingress,omitempty" protobuf:"bytes,3,opt,name=ingress"`
	// Conditions represents the latest available observations of a Bastion's current state.
	// +patchMergeKey=type
	// +patchStrategy=merge
	// +optional
	Conditions []Condition `json:"conditions,omitempty" patchStrategy:"merge" patchMergeKey:"type" protobuf:"bytes,4,rep,name=conditions"`
	// Last time the condition transitioned from one status to another.
	LastHeartbeatTimestamp metav1.Time `json:"lastHeartbeatTimestamp" protobuf:"bytes,5,opt,name=lastHeartbeatTimestamp"`
	// Last time the condition was updated.
	ExpirationTimestamp metav1.Time `json:"expirationTimestamp" protobuf:"bytes,6,opt,name=expirationTimestamp"`
}
