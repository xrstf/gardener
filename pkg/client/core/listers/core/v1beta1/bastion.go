...
type BastionLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.Bastion, err error)
...
// List lists all Bastions in the indexer.
func (s *bastionLister) List(selector labels.Selector) (ret []*v1beta1.Bastion, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.Bastion))
	})
	return ret, err
}
...
type BastionNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.Bastion, error)
...
// List lists all Bastions in the indexer for a given namespace.
func (s bastionNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.Bastion, err error) {
...
// Get retrieves the Bastion from the indexer for a given namespace and name.
func (s bastionNamespaceLister) Get(name string) (*v1beta1.Bastion, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("bastion"), name)
	}
	return obj.(*v1beta1.Bastion), nil
}
